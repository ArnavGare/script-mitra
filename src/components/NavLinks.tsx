      ...
      {navLinks.map((link) => (
        <li key={link.name}>
          <button
            type="button"
            className={`
              relative px-2 py-1 sm:px-3 font-semibold text-[1.01rem]
              tracking-wide font-inter transition-all duration-300
              group
            `}
          ...
          >
            ...
            <span
              className={`
                absolute left-0 bottom-1 w-0 h-[2px] bg-gradient-to-r from-cyan-400 to-purple-400 rounded-full transition-all duration-300 group-hover:w-full
              `}
            />
          </button>
        </li>
