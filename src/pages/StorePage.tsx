...
export default function StorePage() {
  ...
  const filteredProducts = selectedCategory === "All" ? products : products.filter(p => p.category.toLowerCase() === selectedCategory.toLowerCase() || selectedCategory === "PDFs" && p.tag === "PDF");
  ...
  return <>
      ...
      <section className="max-w-5xl mx-auto w-full mt-8 px-4">
        <div className="flex flex-wrap gap-3 items-center justify-center pb-4">
          {categories.map(cat => <Button key={cat} variant={selectedCategory === cat ? "default" : "outline"} size="sm" onClick={() => setSelectedCategory(cat)} className={clsx("rounded-full px-5 font-medium text-sm transition-shadow duration-200", selectedCategory === cat ? "bg-gradient-to-r from-blue-600 to-purple-500 text-white shadow-lg" : "bg-white/20 border-cyan-100/15 text-blue-800 dark:text-white hover:bg-blue-100/60")}>
              {cat}
            </Button>)}
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto w-full px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-10">
          {popularResources.map((prod, i) => <ProductCard key={prod.title} item={prod} delay={i * 0.04} />)}
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto w-full px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-7">
          {filteredProducts.map((prod, i) => <ProductCard key={prod.title} item={prod} delay={i * 0.03} />)}
        </div>
      </section>
...
function ProductCard({
  ...
  return <div className={clsx("group relative flex flex-col items-center justify-between h-full min-h-[230px] bg-white/80 dark:bg-gray-800/70 border border-blue-100 dark:border-blue-800 rounded-2xl shadow-smooth hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 cursor-pointer overflow-hidden", "backdrop-blur-md")} style={{
    ...
    animationDelay: `${delay}s`
      ...
      <div className="mt-6 mb-3 flex items-center justify-center w-16 h-16 rounded-xl bg-gradient-to-br from-blue-100/80 to-purple-100/70 dark:from-blue-800/60 dark:to-purple-800/50">
        {item.icon ? <item.icon className="w-8 h-8 text-blue-600 dark:text-purple-200 drop-shadow" /> : <ImageIcon className="w-8 h-8 text-blue-400 dark:text-purple-300" />}
      </div>
      ...
      <div className="w-full px-6 pb-5 flex justify-center">
        <Button variant="outline" className="w-full rounded-xl border-blue-200 dark:border-blue-700 hover:border-blue-400 group-hover:bg-blue-50/80 dark:group-hover:bg-blue-900/70 text-blue-800 dark:text-white font-semibold flex items-center justify-center gap-2 transition-transform hover:scale-105">
          <span>Preview</span>
